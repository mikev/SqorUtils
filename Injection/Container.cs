...
using System.Text;
using Sqor.Utils.Injection.Scopes;

namespace Sqor.Utils.Injection
{
    public class Container : Context
    {
        private static MethodInfo genericRegister = typeof(Container).GetRuntimeMethod("Register", new[] { typeof(IResolver), typeof(IScope) });
        private static MethodInfo genericCreateRequest = typeof(Container).GetRuntimeMethod("CreateRequest", new[] { typeof(Context), typeof(Request) });
        private static MethodInfo genericGet = typeof(Container).GetRuntimeMethod("Get", new[] { typeof(IDictionary<Type, IBinding>) });
...
*/
        ...
        {
            return !type.GetTypeInfo().IsPrimitive && !type.GetTypeInfo().IsAbstract && !type.GetTypeInfo().IsInterface && type.GetTypeInfo().DeclaredConstructors.Any();
        }
...
// ReSharper restore UnusedTypeParameter
        ...
        private static class DefaultBinding<T>
        {
            ...
            {
                if (typeof(T).GetTypeInfo().IsGenericType && typeof(T).GetGenericTypeDefinition() == typeof(Func<,>))
                {
                    var returnType = typeof(T).GenericTypeArguments[1];
                    return new TypeFactoryResolver(returnType);
                }
                if (typeof(T).GetTypeInfo().IsGenericType && typeof(T).GetGenericTypeDefinition() == typeof(Func<>))
                {
                    var returnType = typeof(T).GenericTypeArguments[0];
